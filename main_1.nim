...
text = (text & ' ').replace("е", "ё")#заменяем все вхождения е на ё и добавляем пробел в конец, дабы подвести всё под шаблон '. ' - разделитель предложений
var sentencesTheText = text.split(". ")
for i in 0..sentencesTheText.len - 1:#после разделения разворачиваем пред. и добавляем точку в начало(предложение обязано ей кончатся)
    sentencesTheText[i] = '.' & sentencesTheText[i].reversed
text = sentencesTheText[0..sentencesTheText.len - 2].join("\n")#собираем в текст под нужный формат
